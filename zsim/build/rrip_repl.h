...
class SRRIPReplPolicy : public ReplPolicy {
    ...
    public:
        ...
        //  Constructor created from init.cpp
         ...
         template <typename C> inline uint32_t rank(const MemReq* req, C cands) override {
            uint32_t replacement_candidate = -1; // initialing replacement_candidate_indx
            ...
            while(1) // loop it until found first srrip_value equals to 3
            {
             for (auto ci = cands.begin(); ci != cands.end(); ci.inc()) {
                if(srrip_value[*ci] == 3) // search for first srrip_value == 3
                 {
                   replacement_candidate = *ci ;
                   replacement_flag = 1; //set replacement_flag
                   return replacement_candidate;  //Found first 3 in an array replace this index block with new block       
                }
                 
             }
              increment_srrip(cands); // if not found first srrip value equals to 3 increment srrip_value of all the lines
            }
           }
